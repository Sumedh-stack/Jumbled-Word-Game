...
class see_wildlife : AppCompatActivity() {

        lateinit var lists: ArrayList<Subject2>
        lateinit var DB:Sqhelperwildlife
        lateinit var data: Cursor
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_see_wildlife)
            val go =findViewById<Button>(R.id.go4)

            go.setOnClickListener{
                var intent = Intent(this@see_wildlife,WildLife::class.java)
                startActivity(intent)
            }
            lists=ArrayList<Subject2>()
            DB= Sqhelperwildlife(applicationContext)
            data=DB.data_getter
        val adapter2=Adapter2(applicationContext,lists)
        val recyclerView=findViewById<RecyclerView>(R.id.list4)
    ...
    override fun onBackPressed() {
        startActivity(Intent(this@see_wildlife,WildLife::class.java))
        return
    }
