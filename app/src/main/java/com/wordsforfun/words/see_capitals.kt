...
import kotlinx.android.synthetic.main.activity_see_capitals.*
import kotlinx.android.synthetic.main.activity_see_words.*

class see_capitals : AppCompatActivity() {
    lateinit var lists: ArrayList<Subject>
    lateinit var DB:SQHelpercountries
    lateinit var data: Cursor

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_see_capitals)
        val go =findViewById<Button>(R.id.go2)

        go.setOnClickListener{
            var intent = Intent(this@see_capitals,Countries::class.java)
            startActivity(intent)
        }
        lists=ArrayList<Subject>()
        DB= SQHelpercountries(applicationContext)
        data=DB.data_getter


        val adapter2=Adapter(applicationContext,lists)
        val recyclerView=findViewById<RecyclerView>(R.id.list2)
    ...
    override fun onBackPressed() {
        startActivity(Intent(this@see_capitals,Countries::class.java))
        return
    }
